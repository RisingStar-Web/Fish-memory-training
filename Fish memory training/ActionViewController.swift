...
class ActionViewController: UIViewController {
    ...
    func buildScene() {
        ...
        for row in 0..<4 {
            for col in 0..<3 {
                ...
                let rootFlipView = CardView(frame: CGRect(x: x, y: y, width: cardSize, height: cardSize))
                
                let cardBack = getCard(0, 0, cardSize, "card")
                
                // let imgCode = Int(arc4random_uniform(UInt32(maxCount)) + 3)
                let imgCode = a[index]
                
                let imageName = "img\(imgCode)"
                let cardImage = getCard(0, 0, cardSize, imageName)
    ...
    func getCard(_ x: CGFloat, _ y: CGFloat, _ size: CGFloat, _ imgName: String) -> UIImageView {
    ...
    func flipCard(_ card: CardView) {
        ...
        if card.showingBack {
            ...
            let i = openedCards.firstIndex(of: card)!
            if i >= 0 {
        ...
        if openedCards.count == 2 {
            ...
            if (card1.code == card2.code) && (!card1.showingBack) && (!card2.showingBack) {
                ...
                if (card1.tapCount == 1) && (card2.tapCount == 1) {
                    showLabel(text: "Lucky!".localized)
                    incScore(3)
                } else {
                    incScore(2)
                }
    ...
    func showFinal() {
        ...
        if currentLevel <= Data.levels.count {
            showLabel(text: "Congrats!".localized)
            labelLevel.text = "LEVEL".localized + " \(currentLevel)"
        } else {
            showLabel(text: "The end".localized)
            showTheEnd()
        }
